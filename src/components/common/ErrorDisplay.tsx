...
import { AlertTriangle, RefreshCw, Phone, Wifi, WifiOff } from 'lucide-react';
      ...
      <Card className={`${getBackgroundColor()} border-2`}>
        ...
        <CardContent className="space-y-4">
          ...
          {isServerError && (
            <div className="bg-orange-100 border border-orange-300 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-orange-800">Informasi Server</span>
              </div>
              <p className="text-sm text-orange-700">
                Tim kami telah diberitahu tentang masalah ini dan sedang memperbaikinya. 
          ...
          {isAuthError && (
            <div className="bg-yellow-100 border border-yellow-300 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-yellow-800">Sesi Berakhir</span>
              </div>
          ...
          {showTechnicalDetails && process.env.NODE_ENV === 'development' && (
            <details className="bg-gray-100 border border-gray-300 rounded-lg p-4">
              ...
              <div className="text-xs text-gray-600 space-y-1">
                <p><strong>Status Code:</strong> {errorInfo.statusCode || 'N/A'}</p>
                <p><strong>Category:</strong> {errorInfo.category}</p>
                <p><strong>Severity:</strong> {errorInfo.severity}</p>
                <p><strong>Technical Message:</strong> {errorInfo.technicalMessage}</p>
                {error?.originalError && (
          ...
          <div className="text-center text-sm text-gray-600 pt-2">
            {isNetworkError && (
              <p>Jika masalah koneksi berlanjut, silakan hubungi administrator.</p>
            )}
            {isServerError && (
              <p>Masalah server akan segera diperbaiki. Terima kasih atas kesabaran Anda.</p>
            )}
