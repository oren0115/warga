...
const BottomNavigation: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-50">
      <nav className="flex justify-around max-w-full overflow-hidden">
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center py-3 px-1 min-w-0 flex-1 h-18 transition-all duration-200 ${
                isActive(item.path)
            ...
            >
              ...
              <span className={`text-xs mt-1 font-medium truncate ${isActive(item.path) ? 'text-green-700' : ''}`}>
                {item.label}
              </span>
            </button>
          );
