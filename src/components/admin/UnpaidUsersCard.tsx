...
import { AlertCircle, Users, Phone, Home, Calendar, DollarSign, UserX, Filter, X, ChevronDown } from "lucide-react";
...
const UnpaidUsersCard: React.FC<UnpaidUsersCardProps> = ({ className }) => {
  ...
  if (isLoading) {
    return (
      <Card className={`rounded-2xl shadow-md hover:shadow-xl transition ${className}`}>
        <CardHeader>
        ...
        {unpaidUsers.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-500">
              Tidak ada warga yang belum membayar iuran untuk bulan yang dipilih.
            </p>
          </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <Badge 
                        ...
                        className={`text-xs ${
                          user.is_orphaned ? "bg-orange-200 text-orange-800" : ""
                        }`}
                      >
                        {user.username}
                      </Badge>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-600">
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          {user.is_orphaned ? "Rumah N/A" : `Rumah ${user.nomor_rumah}`}
                        </span>
                        {!user.is_orphaned && (
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`font-semibold ${
                          user.is_orphaned ? "text-orange-600" : "text-red-600"
                        }`}>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span>Jatuh tempo: {formatAbsoluteTime(user.due_date)}</span>
                      </div>
                    </div>
                    
                    {user.is_orphaned && (
                      <div className="mt-2 p-2 bg-orange-100 rounded text-xs text-orange-800">
                        <strong>Perhatian:</strong> User ini sudah dihapus dari sistem, 
                        tetapi tagihan masih tersisa. Pertimbangkan untuk menghapus tagihan ini 
                        atau menghubungi admin untuk penanganan lebih lanjut.
                      </div>
                    )}
                  </div>
                </div>
        ...
        {filteredUsers.length > 0 && (
          <div className="mt-4 pt-4 border-t border-gray-200">
            <div className="flex items-center justify-between text-sm text-gray-600">
              <span>
                Total tunggakan {filter !== "all" && `(${filter === "normal" ? "User Aktif" : "User Dihapus"})`}:
              </span>
